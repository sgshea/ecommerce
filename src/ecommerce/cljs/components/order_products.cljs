...
(defonce error-state (r/atom ""))

(defn error-handler [{:keys [status status-text]}]
  (.log js/console (str "error on order post request: " status " " status-text))
  (reset! error-state "Error on order request, make sure not to order more than quantity available."))
...
(defn post-order [order]
  (POST "/api/orders"
    {:headers (conj {"Accept" "application/transit+json"} (get-auth-header))
     :params order
     :handler #(do
                 (.log js/console "Added order: " (clj->js order))
                 (reset! error-state ""))
...
(defn order-products-button-dialog
  ...
  [dialog-open? products selected-products get-products]
